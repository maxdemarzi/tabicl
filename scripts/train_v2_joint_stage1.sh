#!/bin/bash
# TabICLv2 pre-training -- JOINT classifier + regressor (TP-07), Stage 1 (of 3).
# One checkpoint for both tasks, in place of train_v2_{clf,reg}_stage{1,2,3}.sh. Identical to
# the classifier recipe except --multitask True and --num_quantiles 999 (below, after
# --max_classes). Each step draws ceil(64/2) classification and floor(64/2) regression
# datasets, so a step costs what a single-task step costs and one run covers both tasks.
#
# norm_type: the classifier's LayerNorm WITH biases (the default), not the regressor's
# bias-free one. The two recipes differ in it and a shared trunk needs one; biased is a
# superset of bias-free, matches the TP-07 proxy arms, and lets a joint model be
# warm-started from the released classifier (trunk + head) plus the released regressor
# (head only) with TabICL.load_single_task_state_dict.
#
# UNVERIFIED on more than one GPU: the joint step keeps every parameter in every DDP
# backward with a zero-weighted term instead of find_unused_parameters (see _run.py).
# Tested on one device only. Run scripts/cloud/pod_doctor first; on a host that fails
# its NCCL check, use NUM_GPUS=1.
#
...
CKPT_DIR=/path/to/checkpoints/tabiclv2-joint/stage1

torchrun --standalone --nproc_per_node=$NUM_GPUS -m tabicl.train \
            --wandb_log False \
            --wandb_project TabICLv2 \
            --wandb_name tabiclv2_joint_stage1 \
            --device cuda \
            --dtype float32 \
            --np_seed 42 \
            --torch_seed 42 \
