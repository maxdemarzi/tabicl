#!/bin/bash
# TabICLv2 pre-training -- JOINT classifier + regressor (TP-07), Stage 2 (of 3). See
# train_v2_joint_stage1.sh for what differs from the classifier recipe, and
# train_v2_clf_stage1.sh for the full recipe.
#
...
CKPT_DIR=/path/to/checkpoints/tabiclv2-joint/stage2
STAGE1_CKPT=/path/to/checkpoints/tabiclv2-joint/stage1/step-500000.ckpt
...
torchrun --standalone --nproc_per_node=$NUM_GPUS -m tabicl.train \
            ...
            --wandb_name tabiclv2_joint_stage2 \
            --device cuda \
            --dtype float32 \
            --np_seed 42 \
            --torch_seed 42 \
